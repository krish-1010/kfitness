...
export async function PATCH(
  ...
  const [row] = await db.update(supplements).set(patch).where(eq(supplements.id, numericId)).returning();
...
export async function DELETE(
  ...
  await db.update(supplements).set({ archived: true }).where(eq(supplements.id, numericId));
