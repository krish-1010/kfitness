...
**What's next:** Phase 4's own plan file flagged one nice-to-have not built: a visible legend/key explaining the calendar's color meanings (hit/partial/missed) — currently inferable from context but not labeled on-screen. Beyond that, no committed roadmap item remains from the original phase list; ask the user, don't assume. *(Superseded — see next entries for what actually got built next.)*

## ✅ RESOLVED — create-user.ts copies tutorial links; new delete-user.ts (2026-09-21)

User asked "how to add users" / "how to delete a user" — surfaced that `scripts/create-user.ts` already exists (copies foods/supplements/plan/exercises from a template user, default id 1, into a new account) but explicitly skipped copying `exerciseLinks` since the new user's exercises get fresh ids and the links would need remapping. Fixed: exercises are now inserted one at a time via `.returning()` (not a single batched insert) so each new row's id is captured immediately into an `oldId -> newId` map, then `exerciseLinks` are copied with `exerciseId` remapped onto it.

New `scripts/delete-user.ts` (`npx tsx scripts/delete-user.ts <email> [--yes]`) — no table in this schema has a real FK/cascade (by design, documented throughout `schema.ts`), so deleting a user means walking every owning table by hand (`workoutPlans` → `planDays` → `exercises` → `exerciseLinks`, `exerciseLog` → `exerciseSetLog`, plus `workoutSessions`/`weights`/`logItems`/`supplementLog`/`supplements`/`waterLog`/`settings`/`foods`/`users`). Defaults to a dry run (prints row counts per table, deletes nothing) — the `--yes` flag is required to actually run it, since this is irreversible with zero soft-delete anywhere in the schema.

Also discussed (not built): converting the app's per-user-owned-copy model for `exercises`/`foods`/`supplements` into a shared global catalog + personal plan/stack references, motivated by the observation that "Leg Press" and "milk's protein content" are universal facts, not personal data, so today's copy-once-then-diverge-forever model means fixes/additions to the library never reach existing accounts. **Explicitly deferred** — user's call was to keep the simpler copy-on-create model (this is a 2-5-person trusted circle, so drift is an acceptable tradeoff) and revisit a real shared-catalog architecture only if/when the app grows toward a wider/self-serve audience, at which point Google OAuth was also flagged as something to consider together with it. Not scheduled, just noted for whenever that conversation resumes.

## ✅ RESOLVED — Muscles-worked-today diagram: tap to enlarge (2026-09-21)

Small UX fix: the "👁 Muscles worked today" diagram on the Workout page rendered at a fixed `size={80}` (kept small so it doesn't dominate the exercise list) but was too small to actually read on a phone. `MuscleDiagram` (in `src/app/(app)/page.tsx`) now wraps that small preview in a button with a "Tap to enlarge" hint; tapping it opens a modal with a much bigger version (`size={170}`), reusing the exact `.modal-overlay` + `✕`-close pattern already established by `ExerciseDetailModal`. Verified on a 375px mobile viewport — fits with room to spare, closes via both `✕` and tap-outside.

## ✅ RESOLVED — Profile account actions: display name, export, delete account; PWA share target (2026-09-21)

User asked to implement the Profile page's long-inert "MORE (COMING SOON)" placeholder rows (`Change display name`, `Export data`, `Import data`, `Backup & sync`, `Delete account`). Scoped down via a clarifying-questions round before building: **Import data was dropped** (restore-from-file is the riskiest of the five — real potential to corrupt/duplicate live data from a malformed or stale file, not worth it for a 2-5-person app); **Backup & sync was dropped/merged into Export** (Neon already does automatic point-in-time backups on the database itself, so a separate app-level cloud-backup integration — new OAuth, storage credentials, a scheduled job — would be real infrastructure protecting against something already covered); **Delete account requires typing your exact account email** to enable the confirm button, since it's self-service with no admin double-check (unlike `delete-user.ts`, which at least requires deliberately passing `--yes`).

**What shipped**, all in `src/app/(app)/profile/page.tsx`'s new "ACCOUNT" section (the old "MORE (COMING SOON)" section is gone entirely — nothing was left in it once Import/Backup&sync were dropped and the rest became real):
- **Display name**: blur-commit input (same pattern as the Goals inputs from Phase 4), `PATCH /api/me` (new handler alongside the existing `GET`).
- **Export data**: `GET /api/export` (new route) returns one JSON file covering every table the account owns (account info, plans/days, exercises + links, all logged history, weights, foods, supplements, settings) — 15 top-level keys. Triggered via a Blob-URL download, filename `cut-tracker-export-<TODAY()>.json`.
- **Delete account**: `DELETE /api/me` (new handler) — runs the exact same table-walk deletion as `delete-user.ts`, now shared via a new `src/lib/deleteUserData.ts` helper (`scripts/delete-user.ts` was refactored to call it too, so script and self-service route can never drift on which tables get cleaned), then clears the session cookie. Gated behind a modal requiring the typed email to match exactly before the delete button enables (verified via `getComputedStyle`: disabled+40% opacity until match, enabled once typed correctly — not actually triggered against the real account during verification).

**PWA share target** (separate but related ask — "sharing a YouTube link should bring me straight to adding it as a tutorial link, copy-pasting is hectic"): added `share_target` to `src/app/manifest.ts` (`action: "/share-target"`, `method: "GET"`, params mapping `title`/`text`/`url`). **Real platform constraint, told to the user up front: Android/Chromium-only, and only once the PWA is installed to the home screen — iOS Safari has no Web Share Target API at all.** New `src/app/(app)/share-target/page.tsx` reads the shared params, extracts a URL from whichever field actually got populated (some apps send a dedicated `url` param; others, like a typical Instagram share, only fill `text` with something like "Check this out: https://..." — handled via a regex fallback), lets you search/pick which exercise it's for, then POSTs to the already-existing `POST /api/exercises/[id]/links` — no new backend needed for the link-saving part itself. Verified end-to-end with a simulated Instagram-style share (URL only inside `text`, no `url` param): correctly extracted, correctly saved to the selected exercise, confirmed via a direct API read, then the test link was deleted via the API afterward.

Both pieces verified against the live Neon DB and pushed across 3 commits (`9cf3e78`, `9c96406`, plus the earlier `71489ab` diagram fix and `6877f3e` create/delete-user scripts); Vercel deployment `dpl_AKxWiqUsHk7fwYkvuHd6Brs4ZQHf` reached `READY`, `get_runtime_errors` clean.

**What's next:** nothing committed — ask the user. The deferred shared-catalog/Google-OAuth conversation (see the create/delete-user entry above) is the one open thread explicitly flagged for "later," not now.
